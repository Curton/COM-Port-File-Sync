...
/**
 * Panel for resolving binary file conflicts.
 * Extracted from BinaryConflictDialog for use in unified ConflictResolutionDialog.
 */
public class BinaryConflictPanel extends JPanel {
    ...
    public BinaryConflictPanel(ConflictInfo conflict) {
        ...
        JLabel headerLabel = new JLabel("<html><b>Binary File Conflict</b><br/>" +
    ...
    private JPanel createInfoPanel(String title, FileChangeDetector.FileInfo info) {
        ...
        String modTime = info.getLastModified() > 0
                ? sdf.format(new Date(info.getLastModified())) : "Unknown";
        infoGrid.add(new JLabel(modTime));

        infoGrid.add(new JLabel("MD5:"));
        String md5 = info.getMd5() != null ? info.getMd5().substring(0, Math.min(8, info.getMd5().length())) + "..."
                : "N/A (fast mode)";
        infoGrid.add(new JLabel(md5));
    ...
    /** Apply target fixed by resolution: use local -> apply to remote; use remote -> apply to local; skip -> BOTH */
