...
public class ConnectionController {
    ...
    public ConnectionController(JFrame owner,
    ...
    public void initEventHandlers(Runnable onConnectionStatusChangedLabel) {
        ...
        components.getSettingsButton().addActionListener(event -> settingsDialog.showDialog(
                owner,
                settings,
                serialPort,
    ...
    public void attemptAutoConnectOnStartup() {
        if (components.getPortComboBox().getItemCount() == 1) {
            ...
            SwingUtilities.invokeLater(() -> {
                if (!state.isConnected()) {
                    logController.log("Only one COM port found (" + singlePort + "), auto-connecting...");
                    connect();
                }
            });
            return;
        ...
        if (!portAvailable) {
            logController.log("Last used COM port " + lastPort + " not available, skipping auto-connect");
            return;
        }
    ...
    private void connect() {
        ...
        if (selectedPort == null || selectedPort.isEmpty()) {
            JOptionPane.showMessageDialog(owner, "Please select a COM port",
        ...
        if (serialPort.open(selectedPort)) {
            ...
            Thread connectThread = new Thread(() -> {
                boolean connected = syncManager.waitForConnection(FileSyncManager.getInitialConnectTimeoutMs());
                SwingUtilities.invokeLater(() -> {
                    if (connected) {
                        ...
                        components.getDirectionButton().setEnabled(true);
                        updateSyncButtonState.run();
                        logController.log("Connection timeout - other side not responding");
            ...
            JOptionPane.showMessageDialog(owner, "Failed to open " + selectedPort,
                    "Connection Error", JOptionPane.ERROR_MESSAGE);
