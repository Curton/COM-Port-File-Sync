...
public class SharedTextController {
    ...
    public SharedTextController(MainFrameComponents components,
    ...
    public void initEventHandlers() {
        components.getSharedTextArea().getDocument().addDocumentListener(new DocumentListener() {
            @Override
        ...
        components.getSharedTextArea().addMouseListener(new MouseInputAdapter() {
            @Override
        ...
        components.getOverwriteFromClipboardButton().addActionListener(event -> {
            try {
        ...
        components.getAppendFromClipboardButton().addActionListener(event -> {
            try {
                String clipboardText = (String) Toolkit.getDefaultToolkit()
                        .getSystemClipboard().getData(DataFlavor.stringFlavor);
        ...
        components.getCopyFromClipboardButton().addActionListener(event -> {
            String text = components.getSharedTextArea().getText();
            StringSelection selection = new StringSelection(text);
            Toolkit.getDefaultToolkit().getSystemClipboard().setContents(selection, null);
            logController.log("Shared text copied to clipboard");
