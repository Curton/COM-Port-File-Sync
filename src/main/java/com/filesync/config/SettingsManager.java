...
/**
 * Manages application settings persistence using Java Preferences API. Stores COM port
 * configuration including baud rate, data bits, stop bits, and parity.
 */
public class SettingsManager {
    ...
    public static final int[] STOP_BITS_VALUES = {
        SerialPort.ONE_STOP_BIT, SerialPort.ONE_POINT_FIVE_STOP_BITS, SerialPort.TWO_STOP_BITS
    };
    ...
    /**
     * Normalize a folder path for consistent comparison and storage. Trims whitespace and
    ...
    /**
     * Set remembered folder mapping for the given port context. Keeps up to {@link
     * #MAX_REMEMBERED_FOLDER_MAPPINGS} entries in MRU order. Call after successful sync completion.
     */
    public void setRememberedFolderMapping(String port, String senderPath, String receiverPath) {
        ...
        mappings.removeIf(
                mapping ->
                        normalizedSender.equals(mapping[0])
                                && normalizedReceiver.equals(mapping[1]));
    ...
    private static String keyForPort(String port) {
        return PREF_FOLDER_MAPPING_PREFIX
                + (port != null && !port.isEmpty() ? port + "." : "default.");
    }
    ...
    private List<String[]> loadRememberedFolderMappings(String port) {
        ...
        for (int i = 0; i < size; i++) {
            String sender =
                    normalizeFolderPath(prefs.get(key + i + "." + PREF_FOLDER_MAPPING_SENDER, ""));
            String receiver =
                    normalizeFolderPath(
                            prefs.get(key + i + "." + PREF_FOLDER_MAPPING_RECEIVER, ""));
        ...
        boolean mappingsChanged =
                storedSize != normalizedMappings.size()
    ...
    private boolean hasStoredMappingDifferences(
            List<String[]> mappings, List<String[]> normalizedMappings) {
    ...
    /**
     * Check if current mapping matches remembered mapping. Both paths should be normalized before
     * calling.
     */
    public static boolean isMappingMatch(
            String localPath,
            String remotePath,
    ...
    /**
     * Returns true when both local and remote folders are changed from remembered mapping. Useful
     * to treat a full mapping switch as an intentional change.
     */
    public static boolean isBothSidesChangedFromRemembered(
            String localPath,
            String remotePath,
            String rememberedSender,
            String rememberedReceiver) {
    ...
    private void loadRecentFolders() {
        ...
        int size =
                Math.min(
                        MAX_RECENT_FOLDERS, Math.max(0, prefs.getInt(PREF_FOLDER_HISTORY_SIZE, 0)));
        for (int i = 0; i < size; i++) {
    ...
    /**
     * Find the receiver folder path that corresponds to the given sender folder for the specified
     * COM port. Looks up the most recent mapping for that port.
     *
