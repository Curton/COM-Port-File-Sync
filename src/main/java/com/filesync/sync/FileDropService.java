...
public class FileDropService {
    ...
    public FileDropService(SyncProtocol protocol,
    ...
    public void sendDropFile(File file) {
        ...
        if (!file.exists() || !file.isFile()) {
            eventBus.post(new SyncEvent.ErrorEvent("Dropped file is not a valid file: " + file.getName()));
            return;
        }
        ...
        if (syncingSupplier.getAsBoolean() || transferBusySupplier.getAsBoolean()) {
            eventBus.post(new SyncEvent.ErrorEvent("Dropped file transfer blocked while a transfer is in progress"));
            return;
        }
        ...
        try {
            ...
            eventBus.post(new SyncEvent.ErrorEvent("Failed to send dropped file: " + e.getMessage()));
        } finally {
    ...
    public void handleIncomingDropFile(SyncProtocol.Message msg) {
        ...
        if (downloadsDir == null) {
            eventBus.post(new SyncEvent.ErrorEvent("Dropped file transfer failed: unable to resolve Downloads folder"));
            return;
        }
        ...
        try {
            ...
            eventBus.post(new SyncEvent.ErrorEvent("Failed to receive dropped file: " + e.getMessage()));
        } finally {
            transferInProgress.set(false);
        }
    }
    ...
    private File getWindowsKnownDownloadsDirectory() {
        ...
        String command = String.format("reg query \"%s\" /v %s", WINDOWS_USER_SHELL_FOLDERS_KEY, WINDOWS_DOWNLOADS_GUID);
        ...
        try {
            ...
            try (BufferedReader reader = new BufferedReader(
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if (candidateFile != null && candidateFile.exists() && candidateFile.isDirectory()) {
    ...
    private File getXdgDownloadsDirectory(File homeDir) {
        ...
        try {
            try (BufferedReader reader = new BufferedReader(
                    new InputStreamReader(new java.io.FileInputStream(xdgFile), StandardCharsets.UTF_8))) {
                String line;
                while ((line = reader.readLine()) != null) {
                    ...
                    if (xdgDownloads != null && xdgDownloads.exists() && xdgDownloads.isDirectory()) {
                        return xdgDownloads;
                    }
                    break;
                }
