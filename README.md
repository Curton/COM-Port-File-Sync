...
- **Preview Search** - Type an extension (`java`, `txt`) or any part of a file name in the preview's search bar to re-order the list: rows whose **extension** matches move to the top, then rows whose **file name** contains the text, then everything else. Matched text is highlighted in yellow. The search is live (debounced) and case-insensitive
